...
100	! &
	...
	&
	&
!	THIS MODULE'S SOLE PURPOSE IS TO READ THE LABEL OFF OF A DISK &
!	PRESENTED TO BE USED IN A BACKUP OR RESTORE RUN.  IT TRIES THE &
!	1ST, 13TH, LAST, AND 13TH FROM LAST DEVICE CLUSTERS ON THE DISK. &

300	! &
	&
	&
...
1000	ON ERROR GOTO 19000 &
	...
	\ Z$=SYS(CHR$(6%)+CHR$(-21%)+CHR$(255%)) &
		! TEMPORARILY DROP RPIV'S. &

1010	I$="BACLAB	V10.1-A" &
		! SET UP HEADER LINE. &
...
1110	P0%=Z0%(0%,27%) &
	...
	\ GOTO 9000 &
		...
		! GET OUT. &
		! THERE IS NO FAILURE FROM THIS PROGRAM. THAT IS, &
		! BACMNT WILL MAKE ALL THE GOOD BAD DECISIONS. &
	&

9000	&
	&
...
9040	Z0%(0%,0%)=B0% IF B0%<>0% &
	...
	&

10350	! READ A LABEL FROM THE DISK. &

10355	ON ERROR GOTO 10395 &
	\ OPEN D$ AS FILE 6% &
	\ Z0%(P0%,18%)=BUFSIZ(6%) &
	\ FIELD #6%, 84% AS L$ &
	\ R9%=1% &
	\ Z$=SYS(PRIV.ON$) &
	\ L9%=PEEK(PEEK(PEEK(PEEK(520%))+12%)+4%)-1% &
	\ Z$=SYS(PRIV.OFF$) &
	\ T%=1% &
		! OPEN IT, GET THE CLUSTERSIZE, FIELD IT. &
		! READ RECORD 1% FIRST. &
		! SET UP THE TEMPORARY ERROR TRAP. &
		! GET THE LENGTH OF THE DISK. &
		! MAKE THIS THE FIRST TRY. &

10360	GET #6%, RECORD R9% &
	\ GOTO 10390 &
		! TRY TO GET A RECORD. &
...
10370	R9%=13% IF T%=1% &
	...
	\ R9%=R9%-13% IF T%=3% &
	\ T%=T%+1% &
	\ GOTO 10360 UNLESS T%=4% &
	\ T$="THIS DISK IS UNREADABLE!" &
	\ GOSUB 23100 &
	\ L$="BAD DISK" &
		! CHANGE THE RECORD NUMBER FOR EACH OF THE 3 EXTRA TRIES &
		! IF ALL TRIES FAIL, WE SEND OUT A BAD DISK MESSAGE, SET &
		! THE ERROR STRING AND DROP OUT. &

10390	ON ERROR GOTO 19000 &
	\ Z0%(0%,15%)=FNA% &
	\ Z0$(Z0%(0%,15%))=LEFT(L$,64%) &
	\ RETURN &
		! RESET THE ERROR TRAP, PUT THE LABEL INTO THE WORK FILE &
		! AND RETURN. &
...
10400	IF E%=14% THEN &
		T$=D$+" - HUNG OR WRITE LOCKED. RETRY, SKIP VALID." &
			\ R1%=32%+64% &
			\ GOSUB 23100 \ GOSUB 23200 &
			\ GOTO 10355 IF (R2% AND 32%)<>0% &
			\ IF (R2% AND 64%)<>0% THEN T%=4% &
							\ GOTO 10370 &
		! HUNG OR WRITE LOCKED ERRORS CAN BE SKIPPED OR &
		! RETRIED. &

10410	GOTO 10370 &
		! ALL OTHER ERRORS GO THROUGH THE CYCLE. &
	&

19000	&
	&
...
22950	T$=T$+"ERRORS	 : "+NUM1$(E2%)+C9$+C9$ &
