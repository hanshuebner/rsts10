...
21	! VER/ED	EDIT DATE	REASON &
	! KMF 01	18-MAY-84	ADD PRIV CHECK &
...
100	! &
	...
	! &
	! &
	! THIS IS THE INITIALIZATION PROGRAM FOR THE 'BATCH' PACKAGE. &
	! IT IS VERY SIMILAR TO THE PROGRAM USED FOR THE SPOOL PACKAGE, &
...
!	C9$		'CRLF' &
!	COPYCO%		COPY COUNT. &
!	DEFACT%		DEFAULT ACCOUNT. &
!	DATWRD%		SWITCH ARGUMENT VALUE. &
!	DATWRD0%	SWITCH ARGUMENT SIGN. &
!	DELET%		DELETE LOG FILE FLAG. &
!	DELET0%		DELETE/NODELETE VALUE. &
!	E%		ERROR VARIABLE. &
!	E0%		ENTRY TYPE. &
!	ERLEV%		ERROR LEVEL FLAG. &
!	ERLEV0%		ERROR LEVEL SWITCH VALUE. &
!	F%		FOUND FLAG. &
...
!	LOGID%		LOGICAL ID FLAG. &
!	LOGID1%		LOGICAL ID FIRST WORD. &
!	LOGID2%		LOGICAL ID SECOND WORD. &
!	LOGDV1%		LOGICAL DEVICE WORD 1. &
!	LOGDV2%		LOGICAL DEVICE WORD 2. &
!	M0%()		CHANGE ARRAY. &
!	MAXUNT%		MAXIMUM UNIT NUMBER. &
!	PKG.LOC$	DEVICE/PPN OF PACKAGE. &
!	P$		CHAIN MODULE NAME. &
!	P%		POSITION POINTER FOR COMMAND STRING. &
!	P0$		PROMPT. &
!	P0%		CURRENT RECORD POINTER. &
!	P1$		CONTINUATION PROMPT. &
!	P1%		RECORD POINTER. &
!	P2%		USED IN SETTING UP CHAIN LISTS. &
!	PHYCLU%		PHYSICAL DEVICE CLUSTERSIZE. &
!	PHYDHI%		PHYSICAL DEVICE DEVICE HANDLER INDEX WORD. &
!	PHYFLG%		PHYSICAL DEVICE FLAG WORD 2. &
!	PHYMOD%		PHYSICAL DEVICE MODE OF OPEN. &
!	PHYSDV%		PHYSICAL DEVICE SPECIFIED FLAG. &
!	PHYSD1%		WORD 1 OF PHYSICAL DEVICE SNAME. &
!	PHYSD2%		WORD 2 OF PHYSICAL DEVICE NAME. &
!	PHYSIZ%		BUFFERSIZE OF PHYSICAL DEVICE. &
!	Q9%		DESTINATION VOLUME RECORD POINTER. &
!	QUEUE%		QUE SWITCH SPECIFIED FLAG. &
!	QUEUE0%		QUE SWITCH VALUE. &
!	R%		PACKAGE STATUS WORD. &
!	RUNBST%		RUNBURST SWITCH SPECIFIED FLAG. &
!	RUNBS0%		RUNBURST VALUE. &
!	S$		MODULE NAME. &
!	S$()		STRING STACK. &
...
!	UNITNO%		UNIT NUMBER FLAG. &
!	UNITN0%		UNIT NUMBER VALUE. &
!	W$		WORK FILE NAME. &
!	Y%		GCML FLAG WORD. &
!	Y0%		COMMAND FILE DEVICE HANDLER INDEX. &
!	Y1%		LINE COUNT. &
...
!	NAME		LINE	USE &
!	&
!	4000			GET AND PARSE INPUT LINE. &
!	4200			SWITCH PROCESSOR. &
!	10000			INITIALIZE GET COMMAND LINE ROUTINE. &
!	10100			GET COMMAND LINE ROUTINE. &
!	11000			ASSIGN THE DEVICE. &
!	FNM0$		15000	JOB STATUS STRING BUILDER. &
	&

830	! &
	! &
...
1020	PRINT "BATCH	"+I$+CHR$(9%)+ &
		RIGHT(SYS(CHR$(6%)+CHR$(9%)+CHR$(0%)),3%) UNLESS E0% &
		! PRINT THE HEADER LINE PREPARATORY TO GIVING &
		! AN ERROR. &

1025	IF FNPRV%("SWCFG",-1%) = 0% THEN &
		PRINT IF CCPOS(0%) &
...
1030	CHANGE SYS(CHR$(12%)) TO Z% &
	...
	\ IF Z%(3%)+SWAP%(Z%(4%))<>15%*2% THEN &
		PRINT "?BATCH must be COMPILED" &
	\	GOTO 32767 &
		! USE LOGICAL FOR PACKAGE LOCATION &
		! PRINT ERROR IF IT WAS NOT A COMPILED FILE. &
...
4050	PRIOR%,RUNBST%,PHYSDV%,LOGFLE%,LOGID%,ASSGN%,NOTE%,FORM%, &
		HEADNG%,ALIGN%,LNGTH%,WIDTH%,QUEUE%,DELET%,ERLEV%,LPFORM%, &
		TOLFST%,TOPFST%,SKPSIX%=0% &
	\ TOPF0%,QUEUE0%,ASSGN0%=0% &
	\ HEADN0%,COPYC0%=1% &
	\ ERLEV0%=0% &
	\ PRIOR0%=-8% &
	\ RUNBS0%=6% &
...
4110	S%=S%+1% \ S%(S%)=P% &
	...
	\	GOTO 4310 IF FNR%("NOTE",3%)<>0% &
	\ 	GOTO 4350 IF FNR%("QUEUE",3%)<>0% &
	\ 	GOTO 4360 IF FNR%("NOQUEUE",5%)<>0% &
	\	GOTO 4370 IF FNR%("DELETE",3%)<>0% &
	\ 	GOTO 4380 IF FNR%("NODELETE",5%)<>0% &
	\ 	GOTO 4390 IF FNR%("ERROR",3%)<>0% &
	...
	!\	GOTO 4330 IF FNR%("TOLF",3%)<>0% &
	!\	GOTO 4340 IF FNR%("TOPF",3%)<>0% &
...
4350	T$="Queue" &
	...
	\ QUEUE%=-1% &
	\ QUEUE0%=QUEUE0% OR 1% &
	\ GOTO 5110 UNLESS FNR%(":",1%) &
	\ P0%=FNPUSH%(P0%) &
	\ P0%=FNA% &
	\ P%=FNP%(P0%,P%,C$,4095%,16375%) &
	\ Z0%(0%,32%)=Z0%(P0%,6%) UNLESS E% &
	\ Z0%(0%,33%)=Z0%(P0%,7%) UNLESS E% &
	\ Z%=FNA0%(P0%) &
	\ P0%=FNPOP% &
	\ Z%=E% \ E%=0% &
	\ GOTO 5030 IF Z% &
...
4390	T$="Error level" &
	\ GOTO 5010 IF ERLEV% &
	\ GOTO 5020 UNLESS FNR%(":",1%) &
	\ ERLEV%=-1% &
	\ ERLEV0%=-1% &
	\ ERLEV0%=0% IF FNR%("NONE",3%) &
	\ ERLEV0%=37% IF FNR%("WARNING",3%) &
	\ ERLEV0%=63% IF FNR%("FATAL",3%) &
	\ GOTO 5030 IF ERLEV0%=-1% &
	\ GOTO 5110 &
		! ERROR LEVEL - &
	&

...
5050	T$="TUNE privilege required for " +T$ &
	\ GOTO 5090 &
		! TUNE PRIVILEGE REQUIRED WITH /PRIORITY OR /RUNBURST &
...
5160	IF (PHYDHI% AND 256%)=0% THEN &
		S%=S%+1% \ S%(S%)=0% \ GOTO 5040 &
...
6200	Z0%(0%,35%),Z%=FNA% &
	\ Z%=FNP%(Z%,0%,PKG.LOC$+"BATCH.DCD/MO:256",0%,0%) &
	\ Z0%(0%,37%),Z%=FNA% &
	\ Z$=A$+RAD$(LOGID1%)+"J"+J$+".COM<188>/CL:-16" &
	\ Z%=FNP%(Z%,0%,Z$,0%,0%) &
		! SET UP "BATCH.DCD" AND THE CONTROL FILE. &

6210	ERLEV0%=0% UNLESS ERLEV% &
	\ QUEUE0%=QUEUE0% OR 1% UNLESS QUEUE% &
	\ QUEUE0%=QUEUE0% OR 4% UNLESS DELET% &
		! SET UP THE ERROR LEVEL AND QUEUE DEFAULTS IF NECESSARY &
	&

7000	! &
	! &
...
7020	Z0%(0%,40%)=ERLEV0% &
	\ Z0%(0%,41%)=QUEUE0% &
		! SET THE ERROR LEVEL AND QUEUE/NOQUEUE DEFAULT. &
	&

8000	! &
	! &
...
8030	PRINT "DETACHING..."+C9$+CHR$(12%) \ Z$=SYS(CHR$(6%)+CHR$(7%)+ &
				CHR$(128%)) &
	\ R%=R% AND -16385% &
...
8110	PRINT "Error in some operation - restarting" \ PRINT T$ &
	\ Z%=((31000% + 32767% + 1%) AND E0%=2%) &
	\ CHAIN PKG.LOC$+"BATCH" LINE Z% &
...
8510	P0%=FNA% &
	\ Z%=FNP%(P0%,0%,PKG.LOC$+"BATIDL",0%,0%) &
	\ P2%=P0% &
	\ Z0%(P0%,1%)=FNA% &
	\ P0%=Z0%(P0%,1%) &
	\ Z%=FNP%(P0%,0%,PKG.LOC$+"BATCH ",0%,0%) &
	\ Z0%(0%,13%)=P2% &
		! SET UP THE CHAIN TO THE IDLER PROGRAM AND BACK TO &
		!  THE INITIALIZATION PROGRAM. &

8520	P0%=FNA% &
	\ Z%=FNP%(P0%,0%,PKG.LOC$+"BATDEC",0%,0%) &
	\ P2%=P0% &
	\ Z0%(P0%,1%)=FNA% &
	\ P0%=Z0%(P0%,1%) &
	\ Z%=FNP%(P0%,0%,PKG.LOC$+"BATRUN",0%,0%) &
	\ Z0%(P0%,1%)=FNA% &
	\ P0%=Z0%(P0%,1%) &
	\ Z%=FNP%(P0%,0%,PKG.LOC$+"BATIDL",0%,0%) &
	\ Z0%(0%,14%)=P2% &
...
9010	Z0%(0%,11%)=FNA0%(Z0%(0%,11%)) IF Z0%(0%,11%)<>0% &
	\ P0%=Z0%(0%,13%) &
	\ IF P0%<>0% THEN &
		P$=FNU$(P0%,"",-1%,0%) &
	\	Z0%(0%,13%)=Z0%(P0%,1%) &
...
9090	W$=SYS(CHR$(8%)+CVT%$(0%)+FNU$(Z0%(0%,12%),"",-1%,-1%)) IF &
		P0%<>0% &
...
10510	IF (INSTR(1%,Z0$,"'") OR INSTR(1%,Z0$,'"'))=0% THEN GOTO 10580 &
...
32710	IF FATAL.ERR% THEN &
		T$ = "?" + C9$ + "BATCH Program = " + &
		 RAD$(Z0%(0%,51%)) + RAD$(Z0%(0%,52%)) + &
		  "  Version = V" + RAD$(SWAP%(Z0%(0%,53%))) + &
		   "  Edit Level = " + RAD$(SWAP%(Z0%(0%,54%))) + C9$ &
	...
	\	T$=FNM0$(T$,Z0%(0%,27%),Z0%(Z0%(0%,27%),27%),0%,0%)+C9$ &
				IF Z0%(0%,27%) &
	\	T$ = T$ +C9$+ " ***** BATCH WILL GO OFF-LINE *****" + C9$ &
	\	GOSUB 23500
32720	 GOSUB 20320 IF (R% AND 8192%) &
	\ GOSUB 23120 IF (R% AND 4096%) &
	\ Z$=SYS(CHR$(6%)+CHR$(18%)) &
	\ ON ERROR GOTO 32730 &
	\ C$=FNU$(Z0%(0%,37%),NULSTG$,-1%,-1%) &
	\ KILL W$ &
	\ KILL C$ &
	\ GOTO 32740
32730	RESUME 32740
